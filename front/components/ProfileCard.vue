<template>
  <div class="flex flex-col space-y-4 p-4">
    <div class="flex space-x-4 items-center">
      <form @submit.prevent.default enctype="multipart/form-data">
        ...
        <div @click="selectFile" class="cursor-pointer">
          <img v-if="user.avatar" :src="user.avatar" class="w-36 h-36 shrink-0 rounded-full" />
          <img v-else src="@/assets/img/default.png" class="w-36 h-36 shrink-0 rounded-full" alt="Default Avatar" />
        </div>
    ...
    <!-- Карусель для загруженных фотографий -->
  <div class="flex flex-col space-y-4 p-4">
    <div class="flex space-x-4 items-center">
          ...
          <div class="flex transition-transform duration-500" :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
            <div v-for="(photo, index) in photos" :key="photo.id" class="min-w-full flex h-full">
              <img v-if="photo.get_image"
                  :src="photo.get_image"
                  class="w-[100px] h-[100px] rounded-full shrink-0"
                  alt="Фото"
      ...
      <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70" @click.self="closeModal">
        <div class="bg-white p-5 rounded shadow-lg relative">
          ...
          <button @click="closeModal" class="mt-4 bg-gray-200 text-gray-700 p-2 rounded hover:bg-gray-300">
            Закрыть
          </button>
